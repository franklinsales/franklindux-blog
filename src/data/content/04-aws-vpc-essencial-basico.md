...
É importante entender que a VPC existe por região. Você nã  o cria uma VPC global - você cria uma VPC em `sa-east-1` e outra VPC em `us-east-1`. Cada VPC é isolada dentro da região onde foi criada. Recursos dentro de uma VPC só podem se comunicar diretamente com recursos na mesma VPC.
...
├── NAT Gateway - permite que subnets privadas acessem a internet sem serem acessíveis externamente
├── Route Table Pública (associada às subnets públicas) - define rotas para o IGW
├── Route Table Privada (associada às subnets privadas) - define rotas para NAT Gateway
├── Security Groups - regras de firewall para instâncias EC2 e outros recursos
└── Network ACLs - regras de firewall para subnets
````

`NAT (Network Address Translation) Gateway` é um serviço gerenciado pela AWS que permite que recursos em subnets privadas iniciem conexões de saída para a internet (para atualizações, downloads, etc.) sem expor esses recursos a conexões de entrada da internet. 
...
**CIDR** (Classless Inter-Domain Routing) ou em português "Roteamento entre Domínios sem Classe" é uma forma de representar faixas de endereços IP de maneira compacta e flexível. Em vez de usar a notação tradicional de máscara de sub-rede (como `255.255.255.0`), o CIDR utiliza um sufixo `/n` que indica quantos bits do endereço IP são fixos para a rede, deixando os bits restantes para identificar hosts individuais.
...
No exemplo `10.0.0.0/16`, os primeiros 16 bits do endereço IP são fixos para a rede, enquanto os 16 bits restantes podem ser usados para identificar hosts individuais dentro dessa rede.
...
No nosso exemplo inicial, a faixa `10.0.0.0/16` permite até 65.536 endereços IP únicos para hosts dentro da rede. Porque os 16 bits restantes (32 bits totais - 16 bits fixos) podem ser usados para criar combinações de endereços IP, resultando em 2^16 = 65.536 endereços possíveis.
...
Lembre-se: em qualquer subnet, o **primeiro endereço** (endereço de rede) e o **último endereço** (broadcast) são sempre reservados — independente do tamanho do prefixo. Em uma `/24` esses são `.0` e `.255`, mas em uma `/26` seriam `.64` e `.127`, por exemplo. Além disso, a AWS reserva os três endereços seguintes ao endereço de rede (`+1`, `+2`, `+3`) para seus próprios serviços internos, totalizando 5 endereços reservados por subnet.
...
- IPs utilizáveis: 65.531 (65.536 totais - 5 reservados pela AWS)
...
- Total: 256 endereços
- IPs utilizáveis: 251 (256 totais - 5 reservados pela AWS)

**`10.0.1.0/26`:**
- Os primeiros 26 bits são fixos: `10.0.1.0` (três octetos completos e 2 bits do quarto octeto)
- Os últimos 6 bits variam: 0 a 63
- Faixa: `10.0.1.0` até `10.0.1.63`
- Total: 64 endereços
- IPs utilizáveis: 59 (64 totais - 5 reservados pela AWS)
...
- A AWS não aceita VPCs com CIDR `/8` por ser muito grande e propenso a conflitos, mas é importante entender o conceito.
...
```
Organização reserva 10.0.0.0/8 como espaço privado total
para planejamento interno de endereçamento (não é o CIDR de uma VPC)
...
Note que `10.0.0.0/8` nesse exemplo **não é o CIDR de uma VPC**. Ele representa apenas o espaço privado total que a organização decidiu reservar para seu planejamento. Cada VPC individual continua usando um bloco menor aceito pela AWS, como `/16`, `/20` ou `/24`.

Em outras palavras: a AWS não aceita criar uma VPC com CIDR `/8`, mas nada impede que uma organização adote a faixa privada `10.0.0.0/8` como referência conceitual para distribuir blocos menores entre várias VPCs, contas, regiões ou ambientes.

**Exemplo para uma startup pequena:**

Uma startup com cerca de 10 programadores e algo em torno de 20 recursos de infraestrutura normalmente não precisa começar com um `/16`. Um `/24` costuma ser suficiente para o início, desde que haja folga para crescimento e uma divisão mínima entre recursos públicos e privados.

Exemplo:

```
VPC Startup: 10.0.10.0/24   → 256 IPs totais, 251 utilizáveis

Subnet Pública: 10.0.10.0/26     → 64 IPs totais, 59 utilizáveis
	- Bastion host ou VPN
	- Load Balancer
	- NAT Gateway

Subnet Privada App: 10.0.10.64/26    → 64 IPs totais, 59 utilizáveis
	- EC2 / containers / serviços de aplicação

Subnet Privada Dados: 10.0.10.128/26 → 64 IPs totais, 59 utilizáveis
	- RDS
	- Redis / cache

Subnet Reserva: 10.0.10.192/26       → 64 IPs totais, 59 utilizáveis
	- expansão futura
	- ambiente temporário
	- migrações
```

Esse tamanho funciona bem porque 20 recursos é um número baixo para consumo de IP dentro da VPC. EC2, RDS, Load Balancer, NAT Gateway e interfaces de rede consomem IPs. Já serviços como S3 não ficam dentro da sua subnet e não consomem IP da VPC nesse modelo.

Para a maioria das startups pequenas, a decisão prática costuma ser:

- usar `/24` se a expectativa de crescimento for moderada e o ambiente for simples
- usar `/23` se já houver expectativa de expansão rápida, múltiplos ambientes ou mais serviços privados desde o início
...
O NAT Gateway permite que instâncias em subnets **privadas** iniciem conexões de saída para a internet (para baixar atualizações, por exemplo) sem receber um IP público e sem ser acessíveis de entrada. Ele será detalhado em artigos futuros.
...
Essa Route Table Main é criada automaticamente com uma rota `local` para o CIDR da VPC, mas não tem rotas adicionais. Se você quiser que suas subnets sejam públicas, precisa adicionar uma rota `0.0.0.0/0 → igw`.
...
Juntando tudo que foi coberto neste artigo, o fluxo para que um recurso em uma subnet pública acesse a internet é:
...
Com os conceitos deste artigo, você já tem tudo o que precisa para criar a estrutura de rede fundamental de qualquer arquitetura AWS. O exercício prático consiste em criar uma VPC do zero com uma subnet pública e uma privada, um IGW, e as Route Tables corretas.
