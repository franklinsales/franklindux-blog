...
Imagine que você precisa traduzir a frase *"O banco está na margem do rio"*.  Para traduzir a palavra "banco" corretamente, você precisa entender o contexto inteiro da frase — afinal, "banco" pode ser uma instituição financeira *ou* um banco de jardim perto d'água.

Antes de 2017, os programas de processamento de linguagem resolviam isso lendo a frase palavra por palavra, da esquerda para a direita, como uma fita cassete. O problema: ao chegar na última palavra, eles já tinham "esquecido" um pouco das primeiras. Frases longas eram um desastre.

Em 2017, pesquisadores do Google publicaram um artigo chamado *"Attention is All You Need"* e apresentaram o **Transformer**. A ideia revolucionária: em vez de ler sequencialmente, o modelo lê **todas as palavras ao mesmo tempo** e calcula como cada palavra se relaciona com todas as outras. Nada se perde.

### Um pouco mais a fundo

O Transformer é composto por dois blocos principais:

- **Encoder** (codificador): lê o texto de entrada e o transforma em uma representação interna rica de significado.
- **Decoder** (decodificador): usa essa representação para gerar o texto de saída, palavra por palavra.

Modelos como o GPT usam *só* o decoder (são bons em gerar texto). Modelos como o BERT usam *só* o encoder (são bons em entender texto). Modelos de tradução usam os dois.

Internamente, o Transformer é uma pilha de **camadas** (layers). Cada camada é composta por dois mecanismos principais:

1. **Self-Attention** (atenção — veremos no próximo tópico)
2. **Feed-Forward Network** (uma rede neural simples que processa cada posição de forma independente)

Após cada um desses mecanismos, há uma operação chamada **Layer Normalization**, que estabiliza os números para o treinamento não "explodir". Há também **conexões residuais** (shortcuts que somam a entrada com a saída), que ajudam o gradiente a fluir durante o treinamento sem desaparecer.

Um LLM moderno como o GPT-4 tem dezenas dessas camadas empilhadas, com centenas de bilhões de parâmetros distribuídos por elas.
...
O resultado é uma pontuação de relevância (um número entre 0 e 1) para cada par de palavras. Palavras muito relacionadas recebem pontuação alta; palavras sem relação, pontuação baixa. O modelo então usa essas pontuações para criar uma representação mais rica de cada palavra, misturando informações das palavras mais relevantes.

### O mecanismo matemático: Queries, Keys e Values

Por baixo dos panos, o self-attention usa três vetores para cada token:

- **Query (Q):** a "pergunta" do token atual — *"o que preciso saber?"*
- **Key (K):** o "rótulo" de cada token — *"o que eu ofereço?"*
- **Value (V):** a informação real que será passada adiante

A atenção é calculada multiplicando Queries pelas Keys (para obter os pesos de relevância), normalizando com uma função *softmax*, e então multiplicando pelos Values. Em notação matemática:

```
Attention(Q, K, V) = softmax(QKᵀ / √d_k) × V
```

O divisor `√d_k` serve para estabilizar os gradientes durante o treinamento.

### Multi-Head Attention

Na prática, o modelo não faz isso uma vez só — ele faz **múltiplas vezes em paralelo**, com diferentes projeções Q, K e V. Cada "cabeça" de atenção aprende a prestar atenção a aspectos diferentes: uma cabeça pode focar em relações sintáticas (sujeito/verbo), outra em co-referências (pronomes e seus antecedentes), outra em relações semânticas. Os resultados são concatenados e projetados de volta ao espaço original.

Isso é o **Multi-Head Attention** — a razão pela qual os Transformers capturam relações tão ricas e variadas.
...
- Uma parte de palavra: `"transformador"` → `["transfor", "mador"]` → `[892, 211]`
...
Por isso, quando você usa um modelo de linguagem, o número de "palavras" que ele processa é diferente do número de *tokens* — normalmente, 1 palavra ≈ 1,3 tokens em inglês, e um pouco mais em outras línguas.

### Embeddings: transformando tokens em significado

Um token é só um número de índice. Mas um número de índice não carrega nenhum significado. `1423` não diz nada sobre o que "gato" significa ou como ele se relaciona com "felino" ou "animal".
...
- `"rei"` e `"rainha"` ficam próximos.
- `"gato"` e `"felino"` ficam próximos.
- `"gato"` e `"astronauta"` ficam distantes.

Um resultado famoso: se você pegar o vetor de "rei", subtrair o vetor de "homem" e somar o vetor de "mulher", você chega muito perto do vetor de "rainha". Matemática de significado.

### Embeddings posicionais

O Transformer processa todos os tokens ao mesmo tempo — então, por padrão, ele não sabe a *ordem* em que os tokens aparecem. Para resolver isso, são adicionados **embeddings posicionais**: vetores que codificam a posição de cada token na sequência. Assim, o modelo sabe que o primeiro token da frase vem antes do segundo, e assim por diante.
...
Sua frase é quebrada em tokens e cada token é convertido em um índice inteiro.
...
```
[4821, 290, 3124, 466, 15300, 30]
→ 6 vetores de, digamos, 4096 dimensões cada
```
...
### Etapa 3 — Camadas do Transformer (a "caixa preta")
...
Após a última camada, cada posição na sequência tem um vetor altamente contextualizado. Para gerar o próximo token, o modelo pega o vetor da última posição e passa por uma **camada linear** que projeta ele de volta ao tamanho do vocabulário (ex: 50.000 dimensões), seguida de uma **função softmax** que transforma esses valores em probabilidades.

```
[0.00001, 0.00003, ..., 0.312, ..., 0.00002]
                          ↑
                     "Brasília" → maior probabilidade
```

O modelo então escolhe o token com a maior probabilidade (ou faz uma amostragem controlada pelo parâmetro **temperature**). Esse token é adicionado à sequência, e o processo se repete até o modelo gerar um token especial de fim (`<|endoftext|>`) ou atingir o limite máximo.
...
Texto de entrada
  ...
  Tokenizador
      ↓
 Tabela de Embeddings + Embeddings Posicionais
      ↓
┌─────────────────────────────┐
│        Camada 1              │
...
└─────────────────────────────┘
  ...
  Camada Linear (projeção)
    ...
    Softmax
      ↓
 Probabilidades sobre o vocabulário
      ↓
  Token gerado → volta ao início
```
...
- **Redes neurais artificiais** são o alicerce: unidades simples organizadas em camadas que aprendem ajustando pesos via backpropagation.
- **Deep learning** empilha muitas dessas camadas, permitindo aprender hierarquias de padrões cada vez mais abstratos.
- **Tokens e embeddings** traduzem linguagem em matemática.
- **Self-attention** permite que o modelo entenda contexto e relações entre palavras de forma global e simultânea.
- **O Transformer** combina tudo isso em uma arquitetura que resolveu os problemas das redes recorrentes e se tornou a espinha dorsal dos LLMs.
- **O pipeline de geração** converte esse entendimento em texto fluente e coerente, um token de cada vez.

O que parece mágica é, no fundo, álgebra linear aplicada em escala monumental — bilhões de parâmetros aprendidos a partir de trilhões de palavras, numa arquitetura elegante que coube em oito páginas de artigo científico e mudou o mundo.

---

*Referência principal: Vaswani et al., "Attention is All You Need", NeurIPS 2017.*